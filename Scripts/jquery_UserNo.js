...
(function ($) {
    ...
    $.fn.setTextboxList1 = function (options)
    {
        var defaults = {
            queryParam: 'q',
            theme: 'facebook',
            animateDropdown: true,
            propertyToSearch: 'OUCode',
            hintText: "請輸入部門代碼",
            searchingText: '<img src="App_Themes/images/loader.gif" /> Searching...',
            noResultsText: "No Find",
            preventDuplicates: true,
            ...
            minChars: 1,
            tokenFormatter: function (item) {
                return "<li><p>" + item.DeptCode + "</p></li>" //顯示字串
            }
        };
        var options = $.extend(defaults, options);
        var val = $(this).val();
        if (val != "") {
            options.prePopulate = getprePopulateJson1(val);
        }
...
function getprePopulateJson1(input) {
    //var tag = $(this).attr('name');
    var json = [];
    if (input == null) {
        return;
