...
$.TokenList = function (input, url_or_data, settings) {
    ...
    function add_token (item) {
        ...
        if(token_count > 0 && settings.preventDuplicates) {
            ...
            token_list.children().each(function () {
                ...
                //2012-06-25 Modified by Neo
                //э礚猭﹚tokenValue拜肈
                var existindId = "";
                var itemId = "";
                for (var key in existing_data) {
                    if (key == settings.tokenValue) {
                        existindId = existing_data[key];
                    }
                }
                for (var key in item) {
                    if (key == settings.tokenValue) {
                ...
                if (existing_data && existindId === itemId) {
	...
	//2012/08/23э皐癸いゅ程穓碝计砞﹚
    function do_search() {
        var query = input_box.val();
		var isChinese = !/[^\u4e00-\u9fa5]/.test(query);
		var minChars = isChinese ? settings.minChnChars : settings.minChars;
