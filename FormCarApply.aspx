    ...
    CodeFile="FormCarApply.aspx.cs" Inherits="FormCarApply" Debug="true"  validateRequest="false" EnableEventValidation="false"%>
...
<asp:Content ID="Content1" runat="server" ContentPlaceHolderID="ContentPlaceHolder1">
    ...
    <link rel="stylesheet" type="text/css" href="Styles/FormSignAlert.css" />      <%--所有彈框類CSS--%> 
  ...
  <%--  <link rel="stylesheet" type="text/css" href="Styles/FormTable.css" /> --%>
   ...
   <script type="text/javascript">
          ...
          function search() {
            var v = $("#ContentPlaceHolder1_DisManNo").val();
                 if (v == "") {
                     ...
                     var v1 = FormCarApply.GetUserName(v).value;
                     if (v1 != "") {
         ...
         function openDialogApprove() {//通過
             ...
             var Action = "Approved";
             var Approve_nextNodeInfo = FormCarApply.GetNextNodeInfo_dept(formType, formID, userID, Action, formNO, formDate, deptCode).value; //Ajax後臺調用
         ...
         function openDialogReject() {//駁回      
             var Action = "Reject";
             var Reject_nextNodeInfo = FormCarApply.GetNextNodeInfo_dept(formType, formID, userID, Action, formNO, formDate, deptCode).value; //Ajax後臺調用
         ...
         function openDialogSign() { //會簽
             ...
             var CounterSign_nextNodeInfo = FormCarApply.GetNextNodeInfo_dept(formType, formID, userID, Action, formNO, formDate, deptCode).value; //Ajax後臺調用
             var style = 'dialogWidth:670px;dialogHeight:400px;center:yes;edge:raised ;scroll:no;status:no'; //設置對話框風格
         ...
         function openDialogDispatch() { //轉簽
             ...
             var SignForward_nextNodeInfo = FormCarApply.GetNextNodeInfo(formType, formID, userID, Action, formNO, formDate, deptCode).value; //Ajax後臺調用
         ...
         function LoadAttach() {
             ...
             if (formID != "" && formYEAR != "" && formMONTHDAY != "") {
                 atttemp = FormCarApply.GetAttachNameList("006", "01").value; //取得當前表單所包含的附檔欄位清單
                 attfieldnamelist = atttemp.split(";");
                 for (var j = 0; j < attfieldnamelist.length; j++) {            //附檔下載
                     attfieldname = attfieldnamelist[j];
                     fileList2 = FormCarApply.getExistFiles(attfieldname, formYEAR, formMONTHDAY, formID).value;
                     if (fileList2 != "")       //判斷是否取到了附檔
                     {
                         fileName2 = fileList2.split(";");
                         for (var i = 0; i < fileName2.length - 1; i++) {
                             fileExtens2 = FormCarApply.getFilesExtens(fileName2[i]).value;
       ...
       function ForDirverChange(){
          ...
          $("#ContentPlaceHolder1_tmp_DriverName").val(drnamevalue);
           ...
           var driverInfo = FormCarApply.GetDriverInfo(fareavalue,drnamevalue).value;
           if(driverInfo!=""){
            ...
            function setcartypelist()    //設定CarType欄位選項list
                {
                    ...
                    var ctype = FormCarApply.GetCarTypeForArea(selectvalue).value;
             ...
             function setdrivername()    //給司機欄位添加值
                 {    
                      ...
                      var driverInfo = FormCarApply.GetDriverInfoForArea(selectvalue,"").value;
                      var driverInfoArr = driverInfo.split("|||");
                      var dn="";
              ...
              function showfactoryarea() {
                    ...
                    if (selectvalue=="TW")
                    {
                         document.getElementById('<%= th_bc.ClientID %>').style.display = "none"; //台灣要求隱藏兩個欄位:是否並車和點評人員(包含兩個tr,兩個th,兩個td)
                         document.getElementById('<%= td_bc.ClientID %>').style.display = "none";

                         document.getElementById('<%= tr_DisMan.ClientID %>').style.display = "none";
                         document.getElementById('<%= Lth_bc.ClientID %>').style.display = "none"; 
                         document.getElementById('<%= Ltd_bc.ClientID %>').style.display = "none";
                         document.getElementById('<%= Ltr_DisMan.ClientID %>').style.display = "none";

                         document.getElementById('<%= Panel_Feedback1.ClientID %>').style.display = "none";        //反?Panel??                
                       ...
                       {//非TW
                         ...
                         document.getElementById('<%= td_subcartype.ClientID %>').style.display = "none";    //廠區不是台灣,則不顯示子車輛類別
           ...
           function showCartype1()            
            {           
                          ...
                          if (document.getElementById('<%= CarType.ClientID %>').options.length != 0)
                             {
                               ...
                               var ctyselectvalue = cty.options[cty.selectedIndex].value;
                                if(ctyselectvalue=="OfficalCar")                        
                                 {
                                     document.getElementById('<%= td_subcartype.ClientID %>').style.display = "";
                                                                           
                                     if (document.getElementById('<%= Panel_DriverInfo.ClientID %>').style.display!="none")       //?編???信息panel有權?,則???panel所有內容
                                        {
                                          document.getElementById('<%= tr_Carinfo.ClientID %>').style.display = "none";
                                          document.getElementById('<%= table_DisMan.ClientID %>').style.display = "none";
                                          document.getElementById('<%= Cap_CarInfo.ClientID %>').style.display = "none";
                                          document.getElementById('<%= tr_Driverinfo.ClientID %>').style.display = "none";
                                         }
                                     if (document.getElementById('<%= LPanel_DriverInfo.ClientID %>').style.display!="none")      //?查看??信息panel有權?,則???panel所有內容
                                         {
                                          document.getElementById('<%= Ltr_Carinfo.ClientID %>').style.display = "none";
                                          document.getElementById('<%= Ltr_DisMan.ClientID %>').style.display = "none";
                                          document.getElementById('<%= LCap_CarInfo.ClientID %>').style.display = "none";
                                          document.getElementById('<%= Ltr_Driverinfo.ClientID %>').style.display = "none";
                                         } 
                                      if (document.getElementById('<%= Panel_ApplyCtrl_1.ClientID %>').style.display!="none")
                                         {
                                            document.getElementById('<%= Panel_ApplyCtrl_11.ClientID %>').style.display = "";
                                         }
                                      if (document.getElementById('<%= LPanel_ApplyCtrl_1.ClientID %>').style.display!="none")
                                         {
                                  ...
                                  else if(ctyselectvalue=="SendCar")
                                  {
                                      document.getElementById('<%= td_subcartype.ClientID %>').style.display = "none";
                    ...
                    {//非TW
                       document.getElementById('<%= td_subcartype.ClientID %>').style.display = "none";
                       document.getElementById('<%= LSubCarType.ClientID %>').style.display = "none";
                      if (document.getElementById('<%= Panel_DriverInfo.ClientID %>').style.display!="none")
                         {                        
                         document.getElementById('<%= tr_Driverinfo.ClientID %>').style.display = "";
                         }
                 ...
                 function showSubCarType1(){               
                              var AspRadio1 = document.getElementById('<%= SubCarType.ClientID %>');
                              var AspRadio_ListItem1 = AspRadio1.getElementsByTagName('input'); //radiobuttonlist值
                               for (var x = 0; x < AspRadio_ListItem1.length; x++) 
                                 {
                                    if (AspRadio_ListItem1[x].checked)
                                      {
                                         if (AspRadio_ListItem1[x].value == "Toyota")
                                           {
...
<asp:UpdatePanel ID="UpdatePanel2" runat="server">
   ...
   <caption>用車資訊(申請者填寫)</caption> 
        ...
        <td colspan="3"  >
             <asp:DropDownList ID="FactoryArea" runat="server" Width="200px" AutoPostBack="true" 
              onchange="showfactoryarea()" onselectedindexchanged="FactoryArea_SelectedIndexChanged">
      ...
      <tr>
            <th class="style91">
                用車類型<span class="requis">*</span></th>
            <td>
               <%-- <asp:RadioButtonList ID="CarType" runat="server" AppendDataBoundItems="True" 
                    RepeatDirection="Horizontal" Width="200px" OnClick="showCartype()">
                    <asp:ListItem Value="OfficalCar">公務車</asp:ListItem>
                    <asp:ListItem Value="SelfDrive">自駕私家車</asp:ListItem>
                    <asp:ListItem Value="SendCar">派車</asp:ListItem>
                </asp:RadioButtonList>--%>

                <asp:DropDownList ID="CarType" runat="server"  Width="150px" onchange="showCartype()">
                </asp:DropDownList>
            </td>

            <td  colspan="2" id="td_subcartype" runat="server" style="display:none">
                <asp:RadioButtonList ID="SubCarType" runat="server" AppendDataBoundItems="True"
                    RepeatDirection="Horizontal" Width="400px" OnClick="showSubCarType()">
                    <asp:ListItem Value="RCK8351">公務車-RCK8351</asp:ListItem>
                    <asp:ListItem Value="RCK8352">公務車-RCK8352</asp:ListItem>
                    <asp:ListItem Value="Toyota">Toyota主管座車</asp:ListItem>
                </asp:RadioButtonList>
            </td>
       </tr>
       <tr>
           ...
           <td class="style92">
               <asp:TextBox ID="ManList" runat="server" 
                   Width="270px"></asp:TextBox>
           </td>
           <th class="style91">
               用車人數<span class="requis">*</span></th>
           <td class="style92">
               <asp:TextBox ID="ManNum" runat="server" Width="270px"></asp:TextBox>
           </td>
       </tr>
       <tr>
          <th >出發時間<span class="requis">*</span></th>
          <td > 
                    <asp:TextBox ID="Departuretime" runat="server" class="Wdate" 
           ...
           <td>
               <asp:TextBox ID="Prereturntime" runat="server" class="Wdate" onclick="WdatePicker({startDate:'%y-%M-#{%d+1} 17:00:00',dateFmt:'yyyy-MM-dd HH:mm'})" Width="270px"></asp:TextBox>
           </td>         
       </tr>
       <tr>
           <th>
       ...
       <tr>
            ...
            <th >航班號</th>
           <td >
                 <asp:TextBox ID="Plnum" runat="server" Width="270px"></asp:TextBox>
                 </td>
           <th >
                抵達或起飛時間</th>
            <td>
                <asp:TextBox ID="Pldate" runat="server" class="Wdate" 
                    onclick="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm',minDate:'%y-%M-#{%d} #{%H}:%m:%s'})" Width="270px"></asp:TextBox>
            </td>
          </tr>
        ...
        <tr>
            ...
            <td colspan="3">
                <asp:TextBox ID="Destination" runat="server" Height="50px" TextMode="MultiLine" 
       ...
       <tr>
           ...
           <td colspan="3">
               <asp:TextBox ID="Purpose" runat="server" Height="50px" TextMode="MultiLine" 
                   Width="700px"></asp:TextBox>
               <br />
           </td>
       </tr>
   ...
   <caption>用車資訊(申請者填寫)</caption> 
        ...
        <td class="style90"  >
            ...
            <td class="style12">
                ...
                &nbsp;&nbsp;&nbsp;<asp:Label ID="LSubCarType" runat="server"></asp:Label>
            </td>
        </tr>
       <tr>
           <th class="style91">
...
<asp:Panel ID="Panel_ApplyCtrl_11" runat="server">
       ...
       <tr>
         <th>實際行車公里數<span class="requis">*</span><br /> (TW:取車公里數)<span class="requis"><br />
             <br />
             </span></th>
           <td >
        ...
        <tr>
            ...
            <tr>
                <th>
                    還車公里數<br /> (TW廠填)</th>
                <td colspan="3">
                    <asp:TextBox ID="TotalKilometers" runat="server" Width="270px"></asp:TextBox>
                </td>
            </tr>
...
<asp:Panel ID="LPanel_ApplyCtrl_11" runat="server">
       ...
       <tr>
         <th>實際行車公里數<span class="requis">*</span><br /> (TW:取車公里數)</th>
           <td class="style89">
        ...
        <tr>
            ...
            <tr>
                <th>
                    還車公里數<br /> (TW廠填)</th>
                <td colspan="3">
                    <asp:Label ID="LTotalKilometers" runat="server"></asp:Label>
                </td>
            </tr>
    </table>
     </asp:Panel>           
